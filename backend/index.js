...
function gatesForBeatInstance(beat) {
    ...
    if (beat.kind === "recon") {
        const v = beat?.vars?.verification || null;

        // If we're asking for the NAME, gate on the landmark's name (player must say it)
        if (v?.kind === "name") {
            const name =
                beat?.vars?.target?.name ||
                beat?.vars?.drop?.name ||
                beat?.vars?.exfil?.name ||
                null;
            if (name) return { landmark: { name } };
        }

        // Otherwise, keyword gate on the expected phrase (cuisine/material/type etc)
        const phrase = v?.phrase;
        if (phrase) return { keyword: [String(phrase)] };
